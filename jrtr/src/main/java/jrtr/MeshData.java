...
public class MeshData {
	...
	public void loop() {
		...
		// smooth old vertices
		for(Vertex v : vertexTable){
			newVertexList.add(smooth(v));
		}
		
		// add new vertices to table (same order as edges)
		for(Edge e : edgeTable){
			newVertexList.add(createVertexOnEdge(e));
		}
		...
		int i = 0;
		for(Face f : faceTable){
			List<Integer> vertex = findVertices(f);
			
			// triangle containing first old vertex
			index[i] = vertex.get(0); i++;
			index[i] = numOldIndex + edgeTable.indexOf(findEdge(vertex.get(0), vertex.get(1))); i++;
			index[i] = numOldIndex + edgeTable.indexOf(findEdge(vertex.get(0), vertex.get(2))); i++;

			// triangle containing second old vertex
			index[i] = vertex.get(1); i++;
			index[i] = numOldIndex + edgeTable.indexOf(findEdge(vertex.get(1), vertex.get(2))); i++;
			index[i] = numOldIndex + edgeTable.indexOf(findEdge(vertex.get(1), vertex.get(0))); i++;

			// triangle containing third old vertex
			index[i] = vertex.get(2); i++;
			index[i] = numOldIndex + edgeTable.indexOf(findEdge(vertex.get(2), vertex.get(0))); i++;
			index[i] = numOldIndex + edgeTable.indexOf(findEdge(vertex.get(2), vertex.get(1))); i++;	
			
			// middle triangle
			for(Edge e : findEdges(f)){
				index[i] = numOldIndex + edgeTable.indexOf(e); i++;
			}
		}
		
		createMesh(newVertexList, index);
		System.out.println("Loop-algorithm finished!");
	}
	
	private Vertex smooth(Vertex v){
		List<Vertex> vertices = findVertices(v);
		int n = vertices.size();
		float beta = 3/16;
		if(n > 3)
			beta = 3f/(8*n);
		
		Vector3f position = new Vector3f(v.position);
		Vector3f color = new Vector3f(v.color);
		Vector3f normal = new Vector3f(v.normal);
		Vector2f texCoord = new Vector2f(v.texCoord);
		
		position.scale(1/beta-n);
		color.scale(1/beta-n);
		//normal.scale(1/beta-n);
		//texCoord.scale(1/beta-n);
		for(Vertex vk : vertices){
			position.add(vk.position);
			color.add(vk.color);
			//normal.add(vk.normal);
			//texCoord.add(vk.texCoord);
		}
		position.scale(beta);
		color.scale(beta);
		//normal.scale(beta);
		...
		return new Vertex(position, color, normal, texCoord);
	}
	
	private Vertex createVertexOnEdge(Edge e){
		Vector3f position = new Vector3f();
		Vector3f color = new Vector3f();
		Vector3f normal = new Vector3f();
		Vector2f texCoord = new Vector2f();
		
		position.add(vertexTable.get(e.v1).position, vertexTable.get(e.v2).position);
		color.add(vertexTable.get(e.v1).color, vertexTable.get(e.v2).color);
		//normal.add(vertexTable.get(e.v1).normal, vertexTable.get(e.v2).normal);
		//texCoord.add(vertexTable.get(e.v1).texCoord, vertexTable.get(e.v2).texCoord);
		position.scale(3);
		color.scale(3);
		//normal.scale(3);
		//texCoord.scale(3);
		for(int v : findVertices(e.f1))
			if(v != e.v1 && v != e.v2){
				position.add(vertexTable.get(v).position);
				color.add(vertexTable.get(v).color);
				//normal.add(vertexTable.get(v).normal);
				//texCoord.add(vertexTable.get(v).texCoord);
			}
		for(int v : findVertices(e.f2))
			if(v != e.v1 && v != e.v2){
				position.add(vertexTable.get(v).position);
				color.add(vertexTable.get(v).color);
				//normal.add(vertexTable.get(v).normal);
	...
	protected class Vertex {
		...
		public Vertex(Vector3f position, Vector3f color, Vector3f normal, Vector2f texCoord) {
