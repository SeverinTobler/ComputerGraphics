...
public class Bezier {
	...
	public boolean drawCurve(float[] controlPoints){
	//public boolean drawCurve(int numSegments, float[] controlPoints){
		//if((numSegments-1)*3+4 != controlPoints.length/2)
		if((controlPoints.length/2-4)%3 != 0)
			return false;
		int numSegments = (controlPoints.length/2-4)/3+1;
	...
	private Vector4f interpolateTangent(float u){
		float t = u-((int)u);
		if(u == C.length){	// last point
			u = u-1;
			t = 1;
		...
		Vector4f point  = new Vector4f(3*t*t, 2*t, 1, 0);
		C[(int)u].transform(point);
		point.w = 1;
		return point;
	}
